...
import type { SampleType, StatusType, TestResultType } from '@/types/apps/limsTypes'
...
type TestResultWithActionsType = SampleType & {
  id: number
  actions?: string
  testId: number
  testName: string
...
const testStatusObj: TestStatusType = {
  ...
  'In Progress': { title: 'In Progress', color: 'info' },
  Completed: { title: 'Completed', color: 'success' },
  Rejected: { title: 'Rejected', color: 'error' }
}

const columnHelper = createColumnHelper<TestResultWithActionsType>()
...
const TestResultsTable = ({ testData }: { testData?: TestResultType[] }) => {
  ...
  const columns = useMemo<ColumnDef<TestResultWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('status', {
        ...
        cell: info => {
          ...
          const statusInfo = testStatusObj[status] || { title: status, color: 'default' }
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <OptionMenu
                ...
                options={[
                  ...
                  //   icon: 'tabler-x',
                  //   menuItemProps: {
                  //     onClick: () => handleTestReject(test.id),
  ...
  const table = useReactTable<TestResultWithActionsType>({
    data: filteredData as TestResultWithActionsType[],
    columns,
    state: {
      columnVisibility
    },
  ...
  const handleFilterChange = (filters: any) => {
    ...
    if (filters.status) {
      filtered = filtered.filter(item => (item as TestResultWithActionsType).status === filters.status)
    }

    if (filters.testType) {
      filtered = filtered.filter(item => (item as TestResultWithActionsType).testName === filters.testType)
    }

    if (filters.dateRange?.start && filters.dateRange?.end) {
      filtered = filtered.filter(item => {
        const date = new Date((item as TestResultWithActionsType).registrationDateTime)
  ...
  const openSampleDetails = (row: TestResultWithActionsType) => {
  ...
  const printBarcode = (row: TestResultWithActionsType) => {
  ...
  const openRemarkDialog = (row: TestResultWithActionsType) => {
    setSelectedTestId(row.id);
    setRemarkDialogOpen(true);
  };

  const openOutsourceDialog = (row: TestResultWithActionsType) => {
    // Example: Open an outsource dialog or trigger outsource logic
    toast.info(`Outsource sample with ID: ${row.sampleId}`);
  };

  const openValidateSample = (row: TestResultWithActionsType) => {
  ...
  return (
    <Card>
      ...
      <BarcodePrintDialog
        ...
        sampleId={(selectedTest as TestResultWithActionsType)?.sampleId || 0}
        barcodeId={(selectedTest as TestResultWithActionsType)?.sampleId ? String((selectedTest as TestResultWithActionsType).sampleId) : undefined}
      />
      <SampleDetailsDialog
        open={showSampleDetails}
        onClose={() => setShowSampleDetails(false)}
        sample={selectedSampleForDetails as TestResultWithActionsType}
      />
      <ConfirmDialog
        ...
        description={
          <div className='flex flex-col gap-4'>
            <Typography>
              Do you want to reject test for sample {(selectedTestForReject as TestResultWithActionsType)?.sampleId}?
            </Typography>
            <CustomTextField
              fullWidth
              multiline
