...
const InstrumentListTable = ({ instrumentData = [], onDataChange }: Props) => {
  ...
  const handleExport = async () => {
    setIsExporting(true)
    try {
      await instrumentService.downloadFile('CSV');
  ...
  const handlePdfExport = async () => {
    ...
    try {
      ...
      autoTable(doc, {
        head: [['Performed On', 'instrumentId', 'instrumentName', 'Performed By', 'instrumentSerialNumber', 'isActive', 'categoryName']],
        body: tableData,
        startY: 30,
        styles: { fontSize: 8 },
  ...
  const handleReasonSubmit = async (reason: string) => {
    ...
    try {
      if (pendingAction === 'update') {
        ...
        await instrumentService.updateInstrument(pendingData.instrumentId, {
          ...pendingData,
          reason: reason
        });
        ...
        await instrumentService.deleteInstrument(pendingData.instrumentId, reason);
  ...
  const handleFilterChange = (key: keyof typeof filters, value: string | Date | { from: Date | null; to: Date | null }) => {
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      <Dialog open={infoDialogOpen} onClose={() => setInfoDialogOpen(false)}>
        ...
        <DialogContent>
          {selectedInstrument && (
            <Box sx={{ pt: 2 }}>
              <Typography><strong>Instrument ID:</strong> {selectedInstrument.instrumentId}</Typography>
              <Typography><strong>Name:</strong> {selectedInstrument.instrumentName}</Typography>
              {/* <Typography><strong>Model:</strong> {selectedInstrument.model}</Typography> */}
              <Typography><strong>Performed By:</strong> {selectedInstrument.updatedBy}</Typography>
              <Typography><strong>Serial Number:</strong> {selectedInstrument.instrumentSerialNumber}</Typography>
              <Typography><strong>Status:</strong> {selectedInstrument.isActive ? 'Active' : 'Inactive'}</Typography>
              <Typography><strong>Category:</strong> {selectedInstrument.categoryName}</Typography>
              <Typography><strong>Port:</strong> {selectedInstrument.port || '-'}</Typography>
              <Typography><strong>IP Address:</strong> {selectedInstrument.ipAddress || '-'}</Typography>
              <Typography><strong>Last Calibration:</strong> {formatDate(selectedInstrument.updatedOn)}</Typography>
            </Box>
          )}
        </DialogContent>
        <DialogActions>
