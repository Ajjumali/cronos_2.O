...
const SampleRegistrationListTable = ({ sampleData = [] }: Props) => {
  ...
  const [selectedAuditTrail, setSelectedAuditTrail] = useState<Array<{
    action: string;
    timestamp: string;
    user: string;
    details: string;
  ...
  const handlePdfExport = async () => {
    ...
    try {
      ...
      // Add table
      autoTable(doc, {
        head: [['Name', 'Sample ID', 'Subject ID', 'Type', 'Registration Date/Time', 'Status', 'Company', 'Branch', 'Laboratory', 'Department']],
        body: tableData,
        startY: 30,
        styles: { fontSize: 8 },
  ...
  return (
    <>
      ...
      >
        ...
        <CardHeader 
          title='Sample Registration List'
          action={
        ...
        />
        <TableFilters 
          setData={(newData) => setData(newData as SampleRegistrationType[])} 
          ...
          {dialogType === 'info' && (
            <Box sx={{ p: 2 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Typography><strong>Sample ID:</strong> {selectedSample?.sampleId}</Typography>
                  <Typography><strong>Registration Date:</strong> {selectedSample?.registrationDateTime}</Typography>
                  <Typography><strong>Subject ID:</strong> {selectedSample?.subjectId}</Typography>
                  <Typography><strong>Name:</strong> {selectedSample?.name}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography><strong>Gender:</strong> {selectedSample?.gender}</Typography>
                  <Typography><strong>Company:</strong> {selectedSample?.company}</Typography>
                  <Typography><strong>Branch:</strong> {selectedSample?.branch}</Typography>
                  <Typography><strong>Type:</strong> {selectedSample?.type}</Typography>
                </Grid>
              </Grid>
            </Box>
          )}
        ...
        <DialogContent>
          {selectedRowForEdit && (
            <SampleRegistrationForm
              initialData={selectedRowForEdit}
              mode="edit"
              onClose={handleCloseEditDialog}
            />
          )}
